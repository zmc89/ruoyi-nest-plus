...
<script setup>
import { listpackage, addpackage, getpackage, getTenantPackageMenu, updatepackage,changePackageStatus } from '@/api/system/package'
...
const tenantPackageFormRef = ref(null)
...
/** 根据租户套餐ID查询菜单树结构 */
const getPackageMenuTreeselect = async (packageId) => {
  const res = await getTenantPackageMenu(packageId)
...
const getList = async () => {
  ...
  tenantPackageList.value = res.data?.list
...
const handleStatusChange = async (row) => {
  ...
  proxy?.$modal.confirm('确认要"' + text + '""' + row.packageName + '"套餐吗？').then(async (res) => {
...
const handleUpdate = async (row) => {
  ...
  const response = await getpackage(_packageId)
  form.value = response.data
  form.value.menuCheckStrictly = response.data.menuCheckStrictly == 1 ? true : false
...
/** 提交按钮 */
const submitForm = () => {
  tenantPackageFormRef.value?.validate(async (valid) => {
    if (valid) {
      buttonLoading.value = true
      form.value.menuIds = getMenuAllCheckedKeys()
      form.value.menuCheckStrictly = form.value.menuCheckStrictly ? 1 : 0
      if (form.value.packageId != null) {
        await updatepackage(form.value).finally(() => (buttonLoading.value = false))
      } else {
        await addpackage(form.value).finally(() => (buttonLoading.value = false))
      }
